...
for number, pred in enumerate(predictors[0:]):
	...
	for num, ids in enumerate(sets[0:]):
		...
		for K,k in enumerate(ids[0:]):
			...
			if pred == 'RNAfold':
				pred_contacts_matrix = RNAfold_bp_prob(k, seq)
			elif pred == 'LinearPartition':
				pred_contacts_matrix = LinearPartition(k, seq)
			elif pred == 'CentroidFold':
				pred_contacts_matrix = CentroidFold_mfe_bp_prob(k, seq)
			elif pred == 'SPOT-RNA':
				pred_contacts_matrix = spotrna(k, seq)
			elif pred == 'SPOT-RNA2':
				pred_contacts_matrix = spotrna2(k, seq)
			elif pred == 'GREMLIN':
				pred_contacts_matrix = GREMLIN_prob(k, seq)
			elif pred == 'PLMC':
				pred_contacts_matrix = plmc_prob(k, seq)
			elif pred == 'mfDCA':
				pred_contacts_matrix = mfdca_prob(k, seq)
			elif pred == 'plmDCA':
				pred_contacts_matrix = plmdca_prob(k, seq)
			elif pred == 'SPOT-RNA-2D-Single':
				pred_contacts_matrix = spotrna_2d_single(k, seq)
			elif pred == 'SPOT-RNA-2D':
				pred_contacts_matrix = spotrna_2d(k, seq)
